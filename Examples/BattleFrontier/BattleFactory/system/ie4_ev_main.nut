// ie4_ev_main.nut already exists in the base game, 
// I don't want to put online the code that was made by L5 so 
// I'm only putting the code I added in this file
...
BattleFrontier <- {
    ...
    BattleFactoryWin = 0,
	BattleFactoryTeam = [],
	BattleFactoryRun = false,
	BattleFactoryPlayerLocation = [0, 0, 0, 0],
};

// Battle Factory
class Hissatsu {
    constructor(_name = "", _difficulty = "", _position = "", _crc32 = 0x0, _level = 1) {
        name = _name;
...
class Player {
    constructor(_name = "", _position = "", _hissatsus = []) {
        name = _name;
		position = _position;
		hissatsus = _hissatsus;
    }
...
function removeElementList(list, element) {
    local index = indexOfList(list, element);
...
function generateHissatsuList(poste, difficulty) {
    ...
    // Add skills until the minimum condition is met
	if (globalDifficulty == "easy") {
		...
		local skillOdd = getRandomKeyWithWeights(skill_weights);
		if (skillOdd == "getSkill") {
			local matchHissatsus = skills;
 ...
 * @param {array} players - The list of available players.
 * @param {string} difficulty - The difficulty level.
 * @param {int} number - The number of created team.
 * @return {array} team - The created team.
 */
function CreateTeam(players, difficulty, number) {
    local team = [];
...
findHissatsuByCRC32 <- function (crc32) {
	foreach(hissatsu in defenses) {
		if (hissatsu.crc32 == crc32) {
			return hissatsu;
		}
	}
...
SaveReserve <- function ()
{
	foreach (player in players) {
		if (CMND_GET_PARTY_HANDLE(player.name)) {
			BattleFrontier.Reserve.append(CMND_GET_PARTY_HANDLE(player.name));
